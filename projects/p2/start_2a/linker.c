    ...
    for (int i = 0; i < numFiles - 1; i++)
    {
        for (int j = 0; j < files[i].symbolSize; j++)
        {
            for (int k = i + 1; k < numFiles; k++)
            {
                ...
                {
                    bool sameLabel = (!strcmp(files[i].symbolTable[j].label, files[k].symbolTable[l].label));
                    bool bothDefined = files[i].symbolTable[j].type != 'U' && files[k].symbolTable[l].type != 'U';
